...
**Статус (2026-06-15):** реализация + финализация + production hardening завершены. Backend на
FastAPI + SQLAlchemy + Alembic (`app/`), три модуля + **JWT/RBAC (ADR-009)**, ключи HMAC+pepper+
ротация, CONFLICT/409, фикс гонки (§45), rate-limit/queue/observability за абстракциями (ADR-010).
Весь POM-контур **green (61/61)**. Логин по умолчанию `admin@example.com` / `admin12345` → JWT.
Прод-бэкенды (Redis/Arq/Sentry) опциональны (`requirements-optional.txt`). API-доки: Swagger
`/docs`, `docs/openapi.json`, Postman, примеры/ответы §43–52 в `docs/`. Ветка
